...
`data/sample_issuers.csv` and `data/sample_spreads.csv` are **entirely
synthetic, illustrative data — not real, not live, and not a claim about
any real company.** The 16 issuer names (Atlas Motors Credit, Aurora Cruise
Lines, etc.) are fictional by design, spanning 15 sectors (Autos, Energy,
Telecom, Leisure, Industrials, Retail, Media, Financials, Utilities, Real
Estate, Technology, Healthcare, Metals & Mining, Chemicals, Consumer
Staples) so the universe looks like a real desk's coverage list. Fundamentals
are hand-set to produce a realistic mix of severities; the spread history is
a seeded random walk with two deliberate stress/relief events, generated by
`data/generate_sample_spreads.py`. This exists so the tool runs and is
demoable offline, with no market data subscription, API key, or claim of
